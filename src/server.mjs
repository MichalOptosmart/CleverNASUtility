...
const server=createServer(async(req,res)=>{try{const u=new URL(req.url,'http://localhost'); if(API_TOKEN && u.pathname!=='/health' && req.headers.authorization!==`Bearer ${API_TOKEN}`) return json(res,401,{error:'unauthorized'});
